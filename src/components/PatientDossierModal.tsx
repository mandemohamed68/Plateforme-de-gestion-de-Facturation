  ...
  return (
    ...
    >
      <div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
        {/* Backdrop backdrop */}
        <div 
          className="fixed inset-0 transition-opacity bg-slate-900/40" 
          aria-hidden="true"
          onClick={onClose}
        />

        {/* Modal panel */}
        <div className="inline-block w-full max-w-4xl my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-2xl rounded-3xl border border-slate-200 relative z-10">
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-slate-50">
            <div className="flex items-center space-x-3">
              <div 
                ...
                className="p-2.5 rounded-lg text-white shadow-md shadow-slate-200"
              >
          ...
          <div className="p-6">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-1 space-y-4">
                <div className="bg-slate-50 border border-slate-100 rounded-lg p-4 space-y-3 shadow-xs">
                  <div className="flex items-center space-x-3 border-b border-slate-200 pb-3">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-slate-600 font-mono">ID Int : {patient.id}</p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2.5 text-xs">
                    {patient.birth_date && (
                      <div className="flex items-center text-slate-700">
                        <Calendar className="w-3.5 h-3.5 mr-2 text-slate-500 shrink-0" />
                        <span>Né(e) le : <strong className="text-slate-900 font-black">{patient.birth_date}</strong></span>
                      </div>
                    )}
                    {patient.gender && (
                      <div className="flex items-center text-slate-700">
                        <User className="w-3.5 h-3.5 mr-2 text-slate-500 shrink-0" />
                        <span>Sexe : <strong className="text-slate-900 font-black uppercase">{patient.gender?.toUpperCase() === 'M' ? 'Masculin' : patient.gender?.toUpperCase() === 'F' ? 'Féminin' : patient.gender}</strong></span>
                      </div>
                    )}
                    {patient.phone && (
                      <div className="flex items-center text-slate-700">
                        <Phone className="w-3.5 h-3.5 mr-2 text-slate-500 shrink-0" />
                        <span>Tél : <strong className="text-slate-900 font-black">{patient.phone}</strong></span>
                      </div>
                    )}
                    {patient.email && (
                      <div className="flex items-center text-slate-700">
                        <Mail className="w-3.5 h-3.5 mr-2 text-slate-500 shrink-0" />
                        <span className="truncate">Email : <strong className="text-slate-900 font-black">{patient.email}</strong></span>
                      </div>
                    )}
                    {(patient.street || patient.city) && (
                      <div className="flex items-center text-slate-700">
                        <MapPin className="w-3.5 h-3.5 mr-2 text-slate-500 shrink-0" />
                        <span className="truncate">Adresse : <strong className="text-slate-900 font-black">{[patient.street, patient.city].filter(Boolean).join(', ')}</strong></span>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="bg-amber-50/50 border border-amber-200 rounded-lg p-4">
                  ...
                  <div className="text-xs text-slate-700 space-y-1.5 font-medium leading-relaxed">
                    <p>
                      <span className="text-slate-600">Antécédents / Notes : </span>
                      <span className="text-slate-900 font-black block mt-0.5 text-sm">
                        {(patient as any).comment || 'Aucun antécédent médical signalé.'}
                      </span>
                    </p>
                    {patient.insurance_id && (
                      <p className="border-t border-amber-200/50 pt-1.5 mt-1.5">
                        <span className="text-slate-600">Couverture Assuré : </span>
                        <strong className="text-emerald-900 block font-black">Assurance Active - Matricule : {patient.insurance_policy_number || (patient as any).insurance_card_number || 'N/A'}</strong>
                      </p>
                    )}
                  </div>
                </div>
              ...
              <div className="lg:col-span-2 flex flex-col min-h-[400px]">
                ...
                <div className="flex-1 overflow-y-auto max-h-[350px] pr-1">
                    ...
                    <div className="space-y-3">
                      {patientLabOrders.length === 0 ? (
                        ...
                        patientLabOrders.map((order) => (
                          <div 
                            key={order.id} 
                            className="bg-white border border-slate-200 hover:border-slate-300 rounded-lg p-4 transition shadow-xs space-y-3"
                          >
                              ...
                              <div className="border-t border-slate-100 pt-3">
                                ...
                                <div className="overflow-hidden border border-slate-150 rounded-xl">
                                  <table className="min-w-full divide-y divide-slate-200 text-[11px]">
                                    <thead className="bg-slate-100 text-slate-700 font-black uppercase tracking-tighter">
                                      <tr>
                                        <th className="px-3 py-2 text-left border-r border-slate-200">Analyse</th>
                                        <th className="px-3 py-2 text-center border-r border-slate-200">Valeur</th>
                                        <th className="px-3 py-2 text-left border-r border-slate-200">Norme</th>
                                    ...
                                    <tbody className="divide-y divide-slate-200 bg-white">
                                      {(order as any).results.map((res: any, idx: number) => {
                                        ...
                                        return (
                                          <tr key={idx} className={isAbnormal ? 'bg-rose-50' : 'hover:bg-slate-50'}>
                                            <td className="px-3 py-2 font-bold text-slate-900 border-r border-slate-100">{res.test_name}</td>
                                            <td className="px-3 py-2 text-center font-black text-slate-900 border-r border-slate-100 bg-slate-50/30">{res.value} {res.unit}</td>
                                            <td className="px-3 py-2 text-slate-700 font-mono font-bold border-r border-slate-100">{res.reference_range}</td>
                    ...
                    <div className="space-y-3">
                      {patientMoves.length === 0 ? (
                        ...
                        patientMoves.map((move) => (
                          <div 
                            key={move.id} 
                            className="bg-white border border-slate-200 hover:border-slate-300 rounded-lg p-4 transition shadow-xs flex flex-col md:flex-row justify-between md:items-center gap-4"
                          >
                            <div>
                            ...
                            <button
                              ...
                              className="flex items-center space-x-1.5 self-start md:self-center bg-slate-100 hover:bg-slate-200 text-slate-800 border border-slate-200 font-black text-[10px] uppercase tracking-wider px-3 py-1.5 rounded-lg transition"
                            >
                  ...
                  {activeTab === 'info' && (
                    <div className="bg-slate-50 border border-slate-200/60 rounded-lg p-5 space-y-4 text-xs">
                      ...
                      {patient.insurance_id && (
                        <div className="border-t border-slate-200 pt-3 bg-emerald-50/40 p-3 rounded-lg border border-emerald-100">
          ...
          <div className="flex justify-end items-center px-6 py-4 border-t border-slate-100 bg-slate-50 gap-3">
            <button
              onClick={onClose}
              style={{ backgroundColor: company.primary_color || '#0f172a' }}
              className="px-6 py-2.5 text-xs font-black uppercase tracking-wider text-white rounded-lg shadow-lg shadow-slate-200 transition hover:opacity-90 active:scale-95"
            >
